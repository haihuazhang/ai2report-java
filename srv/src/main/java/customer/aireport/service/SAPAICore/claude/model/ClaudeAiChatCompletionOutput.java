...
import customer.aireport.service.SAPAICore.claude.ClaudeAiClientException;
...
@Beta
public class ClaudeAiChatCompletionOutput extends ClaudeAiCompletionOutput
    implements DeltaAggregatable<ClaudeAiChatCompletionDelta> {
  /** List of result candidates. */
  @JsonProperty("choices")
  @Getter(onMethod_ = @Nonnull)
  private List<ClaudeAiChatCompletionChoice> choices;
   ...
   * @throws ClaudeAiClientException if the content filter filtered the output.
   */
  @Nonnull
  public String getContent() throws ClaudeAiClientException {
  ...
  @Override
  public void addDelta(@Nonnull final ClaudeAiChatCompletionDelta delta) {
    ...
    if (!delta.getChoices().isEmpty()) {
      ...
      for (int i = choices.size(); i < index + 1; i++) {
        choices.add(new ClaudeAiChatCompletionChoice());
      }
